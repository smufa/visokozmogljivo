...
#SBATCH --job-name=gray-scott-cuda
#SBATCH --gpus=1
...
for IMAGE_FILE in data/*.png; do
  ...
  # Run CUDA implementation and redirect output to file
  echo "Running CUDA implementation for $IMAGE_FILE..."
  ./cuda.out --diffusion_rate_a $DIFFUSION_RATE_A --diffusion_rate_b $DIFFUSION_RATE_B --feed_rate $FEED_RATE --kill_rate $KILL_RATE --time_steps $TIME_STEPS --delta_t $DELTA_T --image_file "$IMAGE_FILE"  --output_file "results/cuda${IMAGE_NAME}.png" &> "results/cuda${IMAGE_NAME}.log"
done