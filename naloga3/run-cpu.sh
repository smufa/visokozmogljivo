...
#SBATCH --job-name=gray-scott-cpu
#SBATCH --output=gray_scott_cpu.log
...
for IMAGE_FILE in data/*.png; do
  ...
  # Run CPU implementation and redirect output to file
  echo "Running CPU implementation for $IMAGE_FILE..."
  echo "TIME_STEPS: $TIME_STEPS"
  ./cpu.out --diffusion_rate_a $DIFFUSION_RATE_A --diffusion_rate_b $DIFFUSION_RATE_B --feed_rate $FEED_RATE --kill_rate $KILL_RATE --time_steps "${TIME_STEPS}" --delta_t $DELTA_T --image_file "$IMAGE_FILE" --output_file "results/cpu${IMAGE_NAME}.png" &> "results/cpu${IMAGE_NAME}.log"
done