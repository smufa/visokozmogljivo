...
__global__ void rgb_to_yuv(const gpuImage in_img, gpuImage out_img) {
  ...
  set(out_img, gidx, gidy, 1, at(in_img, gidx, gidy, 0));
}
...
gpuImage allocateImageOnGPU(const Image &img) {
  ...
  cudaError_t err =
      cudaMalloc(&out_img.data, img.getDataSize() * sizeof(float));
  if (err != cudaSuccess) {
  ...
  err = cudaMemcpy(out_img.data, hostData.data(),
                   img.getDataSize() * sizeof(float), cudaMemcpyHostToDevice);
  if (err != cudaSuccess) {
...
Image retrieveImageFromGPU(const gpuImage &gpu_img) {
  ...
  size_t dataSize =
      gpu_img.width * gpu_img.height * gpu_img.channels * sizeof(float);

  // Copy data from device to host
  cudaError_t err = cudaMemcpy(const_cast<float *>(hostData.data()),
                               gpu_img.data, dataSize, cudaMemcpyDeviceToHost);
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    cudaEventRecord(start);
    rgb_to_yuv<<<gridSize, blockSize>>>(imageIn, imageOut);
    cudaEventRecord(stop);
