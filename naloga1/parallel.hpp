  ...
  // for (int strip = 0; strip * strip_size < out.getHeight(); strip++)
  {
    for (int strip = 0; strip < strips; strip++) {
      ...
      // #pragma omp parallel
      {
        ...
        // out.getHeight()); printf("strip from: %d,  \n", strip_from);
#pragma omp parallel for schedule(static)
        for (int triangle_index = 0; triangle_index <= num_triangles;
          ...
          while (triangle_width >
                 0) // decrease triangle width by 2 every time you go up in Y
          {
            // from / to in x axis
            const int from =
                (max_triangle_width * triangle_index + level - strip_from) - 1;
            ...
            for (int i = from; i < to; i++) {
              const float min_previous_layer =
                  std::min(std::min(out.at(0, i - 1, read_layer),
...
#pragma omp parallel for schedule(static)
        ...
        for (int triangle_index = 0; triangle_index <= num_triangles;
          ...
          int global_level =
              strip_from +
              1; // since the bottom layer was computed by up-facing triangle;
                 // +1 is the new level we compute
          const int max_triangle_width = out.getWidth() / num_triangles;
          while (triangle_width <
                 max_triangle_width) // increase the triangle size by 2,
                                     // building upwards.
          {
            // from / to in x axis
            int from =
                triangle_index * max_triangle_width -
                (global_level - strip_from); // calculate the from index
                                             // (negative for first triangle)
            const int to = std::min(from + triangle_width,
                                    out.getWidth()); // calculate real to
            from = std::max(
                from, 0); // correct the from, to work on first triangle too
